...
from ..models import map_rapid_observation
...
def sync_rapid(cfg: dict):
    ...
    try:
        ...
        records = [map_rapid_observation(obs, station_id) for obs in observations]

        # Fill time grid with 5-min intervals
        timestamps = [r["observed_at"] for r in records if r["observed_at"]]
        if timestamps:
            records = _fill_time_grid(records, station_id, interval_minutes=5)
        ...
        logger.info("Rapid sync complete: %d fetched, %d upserted", len(observations), inserted)
...
def _fill_time_grid(records: list[dict], station_id: str, interval_minutes: int) -> list[dict]:
    """Ensure every expected timestamp slot exists (fill gaps with NULL records)."""
    ...
    for r in records:
        ...
        if ts:
            ...
            minute = (ts_parsed.minute // interval_minutes) * interval_minutes
            rounded = ts_parsed.replace(minute=minute, second=0, microsecond=0)
            existing[rounded] = r
